...
public class ProceedingsByName extends Activity implements OnItemClickListener, OnScrollListener, Runnable {
    ...
    public void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(edu.pitt.is.UMAP2015.R.layout.proceedingsby);

        txtOverlay = (TextView) LayoutInflater.from(this).inflate(edu.pitt.is.UMAP2015.R.layout.list_popup_char_hint, null);
        txtOverlay.setVisibility(View.INVISIBLE);
        //WindowManager settings
        ...
        list = (ListView) this.findViewById(edu.pitt.is.UMAP2015.R.id.list); // real ListView
        listview = (ListView) this.findViewById(edu.pitt.is.UMAP2015.R.id.listview); //side ListView

        adapter = new MyListAdapter(this);
        list.setAdapter(adapter);
        list.setOnScrollListener(this);

        ArrayAdapter<String> adapter1 = new ArrayAdapter<String>(this, edu.pitt.is.UMAP2015.R.layout.textview, py);
        listview.setAdapter(adapter1);
    ...
    private class MyListAdapter extends BaseAdapter implements
        ...
        public View getView(final int position, View convertView, ViewGroup parent) {
            ViewHolder holder = null;
            if (convertView == null) {
                convertView = inflater.inflate(edu.pitt.is.UMAP2015.R.layout.list_item, null);
                holder = new ViewHolder();
                holder.firstCharHintTextView = (TextView) convertView.findViewById(edu.pitt.is.UMAP2015.R.id.text_first_char_hint);
                holder.nameTextView = (TextView) convertView.findViewById(edu.pitt.is.UMAP2015.R.id.title);
                holder.nameTextView.setOnClickListener(this);

                holder.time = (TextView) convertView.findViewById(edu.pitt.is.UMAP2015.R.id.time);
                holder.author = (TextView) convertView.findViewById(edu.pitt.is.UMAP2015.R.id.author);
                holder.type = (TextView) convertView.findViewById(edu.pitt.is.UMAP2015.R.id.type);
                //holder.recommend = (TextView) convertView.findViewById(R.id.recommend);
                holder.schedule = (ImageButton) convertView
            ...
            //holder.orderTextView.setText(String.valueOf(position + 1) + "."); 
            if (pList.get(position).scheduled.compareTo("yes") == 0)
                holder.schedule.setImageResource(edu.pitt.is.UMAP2015.R.drawable.yes_schedule);
            else
                holder.schedule.setImageResource(edu.pitt.is.UMAP2015.R.drawable.no_schedule);

            holder.schedule.setOnClickListener(this);
            holder.schedule.setTag(pList.get(position).id + ";" + position);
            if (pList.get(position).starred.compareTo("yes") == 0)
                holder.star.setImageResource(edu.pitt.is.UMAP2015.R.drawable.yes_star);
            else
                holder.star.setImageResource(edu.pitt.is.UMAP2015.R.drawable.no_star);
            ...
            holder.nameTextView.setTextColor(edu.pitt.is.UMAP2015.R.drawable.Red);
            if (pList.get(position).recommended.compareTo("yes") == 0)
                holder.nameTextView.setText(Html.fromHtml(pList.get(position).title + "<font color=\"#ff0000\"> &lt;Recommended&gt; </font>"));
            else
                holder.nameTextView.setText(pList.get(position).title);
